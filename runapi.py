        ...
        if ln['results']:
            ...
            for i in df:
                data.append({
                        ...
                        #'Número de serie': i['serial_number'],
            ...
            df_g = df2.groupby('Código de material').agg({
                'Tipo': 'first',
                'Nombre de material': 'first',  # O cualquier otra agregación que tenga sentido
                'Cantidad': 'sum',
            }).reset_index()

            df_g = df_g.sort_values(by='Tipo')

            merged_df = pd.merge(df_g, stock_maximo, left_on='Código de material', right_on='CODIGO', how='left')
            df_stock = merged_df.drop(columns=['CODIGO', 'MATERIAL'])
            df_stock.fillna(0, inplace=True)
            #df_stock['TECNICOS'] = df_stock['TECNICOS'].astype(int)
            df_stock = df_stock.rename(columns={'TECNICOS': 'Tope'}).astype({'Tope': int})
            #df_stock['diff'] = (df_stock['Tope'] - df_stock['Cantidad']).astype(int)

            df_stock['diff'] = df_stock.apply(lambda i: i['Tope'] - i['Cantidad'] if i['Tope'] != 0 else i['Cantidad'], axis=1)
            equipos = df_stock.loc[df_stock['Tipo'] == 'Equipos', 'Cantidad'].sum()

            #print(df_stock.info())
            #df_stock.to_csv('dataframe.csv', index=False)
